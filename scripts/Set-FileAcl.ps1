...
<#
    .SYNOPSIS
        Holds one file's DACL to exactly a declared descriptor, wholesale.

    .DESCRIPTION
        The native Ansible module grants access-control entries one at a time and never removes
        one it did not write, so a hand-added grant survives every converge -- and for a file that
        carries key material, one surviving grant is an exposed key. Enforcement means owning the
        WHOLE list, and no native module does that, which is what routes this to the role's
        first-class script convention.

        The descriptor is handled as its SDDL string from end to end. The desired form is supplied
        by the caller: protected (no inheritance from the directory), and exactly the principals
        the caller names. Windows normalises an applied descriptor -- P comes back as PAI -- so
        the caller supplies the NORMALISED form, read back from a host after application, and a
        converged file therefore compares equal byte for byte. Apply-and-verify: after a write the
        ...
        Set-FileAcl.pester.ps1 spec, while a consuming role carries files/Set-FileAcl.ps1.stub,
    ...
    .PARAMETER Path
        The file whose DACL is enforced.

    .PARAMETER Sddl
        The desired DACL as its NORMALISED SDDL string -- the form Windows reads back after
        applying, so a converged file compares equal: protected ('D:P' or its read-back 'D:PAI')
        followed by one or more ACEs and nothing else. The caller owns the model per file. Only
        the DACL section is ever read or written; owner, group and audit are left.

    .EXAMPLE
        .\Set-FileAcl.ps1 -Path 'C:\Program Files\OpenVPN\config-auto\aws-ec2-test.ovpn' -Sddl 'D:PAI(A;;FA;;;SY)(A;;FA;;;BA)(A;;FR;;;S-1-5-80-3750127322-3829920079-842246558-474445854-3948885660)'

    .OUTPUTS
        System.String -- standalone, the result object as JSON: after, before, changed,
        check_mode, msg and path. Under the transport it is $Ansible.Result.
#>
...
# touch the file when they differ. Everything platform-bound goes through Get-Acl/Set-Acl, which
# is also what lets the spec exercise this file on a build host with no NTFS.
