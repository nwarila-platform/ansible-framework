...
<#
    Pester spec for Set-FileAcl.ps1 (org pair convention: every script ships with a sibling
    <Name>.pester.ps1; the pester-matrix workflow runs one leg per pair).

    Runs anywhere, Linux CI included: the ACL and principal APIs are Windows resource management
    and refuse to construct on other platforms, so the script confines every platform call to
    Get-Acl and Set-Acl and handles the descriptor purely as its SDDL string. This file stubs
    those two cmdlets around an in-memory descriptor, which exercises the whole decision surface
...
BeforeAll {
  $script:ScriptPath = Join-Path -Path $PSScriptRoot -ChildPath 'Set-FileAcl.ps1'
  $script:Desired = 'D:PAI(A;;FA;;;SY)(A;;FA;;;BA)(A;;FR;;;S-1-5-80-3750127322-3829920079-842246558-474445854-3948885660)'
  $script:Drifted = 'D:AI(A;ID;FA;;;SY)(A;ID;FA;;;BA)(A;ID;0x1200a9;;;BU)'
  $script:FilePath = 'C:\Program Files\OpenVPN\config-auto\aws-ec2-test.ovpn'
  ...
  # In-memory descriptor. $global:FakeSddl is the file's current DACL; Get-Acl hands out an
  # object whose SDDL methods read it and stage a pending write; Set-Acl commits the staged
  # value -- unless the spec sets $global:FakeWriteIgnored, which models a platform that silently
  # keeps its own shape and is exactly the failure the script's verify pass exists for.
  Function Get-Acl {
    [CmdletBinding()]
    Param (
...
Describe 'Set-FileAcl' {
  ...
  AfterAll {
    Remove-AnsibleContext
    Remove-Variable -Name 'FakeSddl', 'FakeReads', 'FakeWrites', 'FakeWriteIgnored', 'FakeMissing', 'FakeSetSection' `
  ...
  Context 'convergence decisions' {
    ...
    It 'reports no change when the DACL is already exact, and never writes' {
      $Result = & $script:ScriptPath -Path $script:FilePath -Sddl $script:Desired | ConvertFrom-Json
    ...
    It 'replaces an inherited, drifted DACL wholesale and verifies the readback' {
    ...
    It 'fails loudly when the write does not take, instead of claiming a change' {
      ...
      { & $script:ScriptPath -Path $script:FilePath -Sddl $script:Desired } | Should -Throw
    }

    It 'fails loudly on a missing file' {
      $global:FakeMissing = $True

      { & $script:ScriptPath -Path 'C:\Absent\profile.ovpn' -Sddl $script:Desired } | Should -Throw
    }

    It 'rejects a descriptor that is not protected, or that is not ACE-shaped, before reading anything' {
      { & $script:ScriptPath -Path $script:FilePath -Sddl 'D:AI(A;;FA;;;SY)' } | Should -Throw
      { & $script:ScriptPath -Path $script:FilePath -Sddl 'D:Pthis-is-not-sddl' } | Should -Throw
      $global:FakeReads | Should -Be 0
    }

    It 'honours a standalone -WhatIf as check mode: reports the would-be change and writes nothing' {
      $global:FakeSddl = $script:Drifted

      $Result = & $script:ScriptPath -Path $script:FilePath -Sddl $script:Desired -WhatIf | ConvertFrom-Json
  ...
  Context '$Ansible transport' {
    ...
    It 'sets Changed=$False explicitly on an already-exact file' {
      $Context = New-AnsibleContext

      & $script:ScriptPath -Path $script:FilePath -Sddl $script:Desired | Out-Null
    ...
    It 'reports the would-be change in check mode without writing' {
      ...
      & $script:ScriptPath -Path $script:FilePath -Sddl $script:Desired -WhatIf | Out-Null
    ...
    It 'reports no change in check mode on an already-exact file, which is what END relies on' {
      $Context = New-AnsibleContext -CheckMode
