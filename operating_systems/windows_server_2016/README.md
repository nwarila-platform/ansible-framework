# `windows_server_2016` role

Brings a newly launched Windows Server 2016 host to the minimum state the ansible-framework suite assumes: PowerShell served over the transport, an elevated session token, and the computer name set before anything tries to join a directory.
...
```yaml
- name: 'Bootstrap | Windows Server 2016 minimum requirements'
  hosts: 'windows'
  gather_facts: false
  tasks:
    - name: 'Bootstrap | Windows Server 2016'
...
- **The release is asserted, not inferred.** Windows Server 2016 is NT 10.0 build 14393, and the product type is asserted as well, so the role states what it supports rather than relying on a build number being unambiguous.
