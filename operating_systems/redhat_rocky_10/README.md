# `redhat_rocky_10` role

Brings a newly provisioned RHEL or Rocky Linux 10 host to the minimum state the ansible-framework suite assumes.
...
```yaml
- name: 'Bootstrap | RHEL or Rocky 10 minimum'
  hosts: 'linux'
  gather_facts: false
  tasks:
    - name: 'Bootstrap | RHEL or Rocky 10'
      ansible.builtin.include_role:
        name: 'redhat_rocky_10'
...
|---|---|---|
| `rhel10_bootstrap_remote_tmp_base` | the shell plugin's `remote_tmp` | Where AnsiballZ stages modules. Must be writable by the login user and exec-capable. |
| `rhel10_bootstrap_prerequisite_roles` | `[]` | Roles to run before the package work. Empty by default; nothing is enabled out of the box. |
| `rhel10_bootstrap_manage_hostname` | `false` | Whether the role sets the system hostname and updates the hosts file. |
