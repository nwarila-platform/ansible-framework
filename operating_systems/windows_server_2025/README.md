# `windows_server_2025` role

Brings a newly launched Windows Server 2025 host to the minimum state the ansible-framework suite assumes: PowerShell served over the transport, an elevated session token, and the computer name set before anything tries to join a directory.
...
```yaml
- name: 'Bootstrap | Windows Server 2025 minimum requirements'
  hosts: 'windows'
  gather_facts: false
  tasks:
    - name: 'Bootstrap | Windows Server 2025'
...
- **The release is asserted, not inferred.** Windows Server 2025 is NT 10.0 build 26100, and the product type is asserted as well, so the role states what it supports rather than relying on a build number being unambiguous.
