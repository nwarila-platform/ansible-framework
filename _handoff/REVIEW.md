...
| WDM-02a | present_windows provider stage 1 — resolve declared disks by unique_id, attach assert, online/writable fixup, fail-closed safety classification (canonical VOLUME bearers, UNKNOWN-first, allowlist {NTFS,FAT,FAT32,exFAT,ReFS}, exact OURS/PRISTINE, FOREIGN default), distinct-number assert; NON-destructive, still fail-closed (mutation → 02b) | REVISE×5 → **AGREE** r6 (12→3→2→2→2→0; silent-green closed, deterministic classifier per Director middle-path blank-safety) | _(build SHA below)_ | **Live pdq-dev:** 3 real disks → PRISTINE, safety pipeline passed, deferred-fail (ok=22 changed=0); FOREIGN(OS disk)/unattached/AWS-deferred/empty-greens correct; **9 synthetic-fact classifier fixtures** all per spec; validate/defaults comment-only. P4 no defects | _(merge SHA below)_ | Middle-path blank-safety (Director); online-before-classify |
